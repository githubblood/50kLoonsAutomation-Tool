...
class FormFiller:
    ...
    def classify_lead_on_site(
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                url = self._target.get("url", "https://lowcreditfinance.com")
    ...
    def _fill_form(
        ...
        *,
        ...
        for step_num in range(0, max_steps):
            ...
            # ── Duplicate via progress jump ──────────────────────────────────
            # In classify mode we only ever fill email + SSN, so the progress
            # bar should still be low (~7-17%). A returning lead is recognised
            # by the site and the form leaps near the end (e.g. 93%). Treat a
            # big early jump as a duplicate regardless of the page title — the
            # final page's heading isn't always one of the keywords below.
            if classify_only and step_num >= 2:
                pct = self._progress_pct(frame)
                if pct >= 45:
                    ...
                    raise FormFillerError(
                        f"Duplicate: progress jumped to {pct}% at step {step_num} "
                        f"(only email/SSN entered)",
                        error_type="duplicate",
                    )

            # Transient verification / loading spinner (classify mode).
            # Wait it out instead of treating it as a form step — the next
            # screen decides Fresh (real question) vs Duplicate (progress jump
            # / offers page).
            if classify_only and step_num >= 2 and any(kw in title for kw in (
            ...
            # Personalised greeting after SSN (e.g. "Kaylynn!"). In classify
            # mode we never enter the name, so the site can only show it if it
            # pulled the lead from existing records → the lead already exists →
            # Duplicate. Guard with a short, non-question title to avoid
            # matching real questions that happen to contain a common name.
            if classify_only and step_num >= 2:
                fname = str(f.get("first_name") or "").strip().lower()
                if (
                    len(fname) >= 3
                    and len(title) <= 30
                    and "?" not in title
                    and re.search(r"\b" + re.escape(fname) + r"\b", title)
                ):
            ...
            if any(kw in title for kw in [
                ...
                # Classify mode never legitimately reaches a completion page
                # (we only enter email + SSN). Arriving here means the lead was
                # recognised and short-circuited to the end → Duplicate.
                if classify_only and step_num >= 2:
                    self._screenshot(page, row_number, "duplicate_auto_jump")
                    log.warning(
                        "form.duplicate_detected",
                        reason="completion_jump",
                        step=step_num,
                        title=title[:80],
                        row=row_number,
                    )
                    raise FormFillerError(
                        f"Duplicate: jumped to completion/offers page at step {step_num}",
                        error_type="duplicate",
                    )
    ...
    def _handle_step(self, frame: Frame, title: str, f: dict) -> str | None:
        ...
        # ── Landing hero (before iframe form) ───────────────────────────────
        if "borrow" in title and ("bad credit" in title or "50,000" in title):
        ...
        if "how much" in title or ("amount" in title and "loan" in title):
            ...
            if not clicked:
                for c in (chip, "$2,000", "$5,000", "$1,000"):
        ...
        if "verification complete" in title:
            time.sleep(3)
            self._click_primary_continue(frame)
            self._wait_title_change(frame, avoid=["verification complete"], timeout=20)
            return "VERIFIED"
        ...
        if "legal name" in title or (
            ...
            if not self._react_fill_any(
                frame,
                ['input[name="fname"]', 'input[name="first_name"]'],
                f["first_name"],
            ):
                self._fill_nth_visible_input(frame, 0, f["first_name"])
            if not self._react_fill_any(
                frame,
                ['input[name="lname"]', 'input[name="last_name"]'],
                f["last_name"],
            ):
        ...
        if "birth" in title or "date of birth" in title or title.startswith("hi "):
            # LowCreditFinance uses separate inputs dob_month/dob_day/dob_year.
            ...
            if not filled:
                if not self._fill(frame, 'input[name="dob"]', f["dob"]):
                    if mm and dd and yyyy and self._count_visible_inputs(frame) >= 3:
                        ...
                        self._fill_any(frame, ["input:visible"], f["dob"])
            time.sleep(1)
            return self._continue(frame)

        # ── Step 6: ZIP ──────────────────────────────────────────────────────
        if "zip" in title:
            self._react_fill_any(frame, ['input[name="zip"]'], f["zip"])
            try:
                frame.evaluate(
                    """() => {
        ...
        if "source" in title and "income" in title:
            return self._chip(frame, f.get("income_source_chip") or "Employed")
        ...
        if "employer" in title or ("employ" in title and "info" in title):
            # Live LCF markup uses floating labels and inputs WITHOUT name=.
            self._fill_labeled_input(frame, "Employer Name", f["employer_name"])
            self._fill_labeled_input(frame, "Job Title", f.get("job_title", "Employee"))
            self._fill_labeled_input(frame, "Employer Phone", f.get("employer_phone") or f["phone"])
            time.sleep(1)
            return self._continue(frame)
    ...
    def _progress_pct(self, frame: Frame) -> int:
        """Return the form progress percentage (0-100), or -1 if unavailable."""
        try:
            pct = frame.evaluate(
                """() => {
                    const fill = document.querySelector('.lcf-progress-fill, [class*="progress-fill"]');
                    if (fill) {
                        const m = (fill.style.width || '').match(/([0-9]+)/);
                        if (m) return parseInt(m[1], 10);
                    }
                    const inline = document.querySelector(
                        '.lcf-progress-percentage-inline, [class*="progress-percentage"]'
                    );
                    if (inline) {
                        const m = (inline.textContent || '').match(/([0-9]+)/);
                        if (m) return parseInt(m[1], 10);
                    }
                    return -1;
                }"""
            )
            return int(pct) if pct is not None else -1
        except Exception:
            return -1
    ...
    def _click_primary_continue(self, frame: Frame) -> bool:
        try:
            return bool(
                frame.evaluate(
                    """() => {
                        const b = document.querySelector('button.lcf-btn-primary');
    ...
    def _parse_fields(self, row: dict) -> dict:
        ...
        income_source_chip = self._map_income_source(g("Income Source", "Source of Income")) or "Employed"

        # Military (chip step)
        active_military_chip = self._map_yes_no_chip(g("Active Military Status", "Active in Military?")) or "No"

        # Next payday (usually a chip choice, not a date input on LCF)
        next_payday_choice = self._map_next_payday_choice(g("Next Payday", "Next Pay Date")) or "Next scheduled"
        ...
        account_age_chip = self._map_account_age(g("Account Age", "Length of Bank Account")) or "More than 2 Years"
        ...
        free_trial_choice_chip = self._map_yes_no_chip(g("FREE Credit Score Trial Choice", "Free Trial")) or "Yes"

        # Unsecured debt amount (Yes/No question on form; decide based on amount)
        unsecured_debt_chip = self._map_unsecured_debt(g("Unsecured Debt Amount", "Debt Amount")) or "No"
