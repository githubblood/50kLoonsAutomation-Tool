"""
Read-only verification harness for BorrowMoney duplicate detection.

Usage:
    python test_classify_bm.py <ROW_NUMBER>

Fetches one row from the BorrowMoney worksheet (Sheet2), runs the live
...
import core.form_filler_borrowmoney as filler_mod


def main() -> None:
    if len(sys.argv) < 2:
        print("usage: python test_classify_bm.py <ROW_NUMBER>")
        sys.exit(1)
    ...
    sheet_url      = os.getenv("SHEET_URL_BORROW_MONEY") or os.getenv("GOOGLE_SHEET_URL", "")
    worksheet_name = os.getenv("SHEET_WS_BORROW_MONEY")  or "Sheet2"
    ...
    offer_config["target"] = {**config.get("target", {}), "url": "https://borrowmoney.us"}
    offer_config.setdefault("screenshots", {})["directory"] = "screenshots/borrow_money"
