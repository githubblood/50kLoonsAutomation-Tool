"""
Read-only verification harness for SuperPersonalFinder duplicate detection.

Usage:
    python test_classify_spf.py <ROW_NUMBER>

Fetches one row from the Super Personal worksheet (Sheet4), runs the live
website classification, prints Fresh / Duplicate. Does NOT write to the sheet.
"""
import os
import sys
...
import core.form_filler_superpersonal as filler_mod


def main() -> None:
    if len(sys.argv) < 2:
        print("usage: python test_classify_spf.py <ROW_NUMBER>")
        sys.exit(1)
    ...
    sheet_url      = os.getenv("SHEET_URL_SUPER_PERSONAL") or os.getenv("GOOGLE_SHEET_URL", "")
    worksheet_name = os.getenv("SHEET_WS_SUPER_PERSONAL")  or "Sheet4"
    ...
    offer_config["target"] = {**config.get("target", {}), "url": "https://superpersonalfinder.com"}
    offer_config.setdefault("screenshots", {})["directory"] = "screenshots/super_personal"
