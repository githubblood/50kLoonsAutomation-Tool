"""
Read-only verification harness for 50k Loans duplicate detection.

Usage:
    python test_classify_50k.py <ROW_NUMBER>

Fetches one row from the 50k worksheet (Sheet1), runs the live website
classification, prints Fresh / Duplicate. Does NOT write to the sheet.
"""
import os
import sys
...
import core.form_filler as filler_mod


def main() -> None:
    if len(sys.argv) < 2:
        print("usage: python test_classify_50k.py <ROW_NUMBER>")
        sys.exit(1)
    ...
    sheet_url      = os.getenv("SHEET_URL_50K") or os.getenv("GOOGLE_SHEET_URL", "")
    worksheet_name = os.getenv("SHEET_WS_50K")  or "Sheet1"
    ...
    offer_config["target"] = {**config.get("target", {}), "url": "https://50kloans.com/"}
    offer_config.setdefault("screenshots", {})["directory"] = "screenshots/50k_loans"
